...
import kr.kymedia.kykaraoke.play.SongUtil;
import kr.kymedia.kykaraoke.tv.BuildConfig;
import kr.kymedia.kykaraoke.tv.play._LyricsPlay;

/**
 * <pre>
 * Copy of {@link kr.kymedia.kykaraoke.play.SongPlayer.PlayView.PlayViewThread}
 * </pre>
 *
 * @author isyoon
 * @version 1.0
...
class KPLyrics extends Thread {
	...
	@Override
	public void run() {
		if (IKaraokeTV.DEBUG) Log.i(_toString(), getMethodName() + mLyricsPlay.m_redraw + ":" + mLyricsPlay.getHolder() + ":" + mLyricsPlay.getHolder().getSurfaceFrame());
	...
	public void onWindowResize(int w, int h) {
		if (IKaraokeTV.DEBUG) Log.i(_toString(), getMethodName() + w + ", " + h);
	...
	protected void drawLyrics(Canvas canvas, int t) {
		...
		if (nState == 1) {
			...
			// if (IKaraokeTV.DEBUG) _LOG.i("BGKIM", "[nPixel] : " + String.valueOf(nPixel));
			...
			switch (nCodeFlag) {
				...
				case _Const.SYNC_TEXT:
					if (!m_bReady) {
						...
						// if (IKaraokeTV.DEBUG) _LOG.i("BGKIM", "[0] : " + m_strLine[0]);
						// if (IKaraokeTV.DEBUG) _LOG.i("BGKIM", "[1] : " + m_strLine[1]);
					...
					if ((mLyricsPlay.m_data.getListSyncTag().get(m_itor).nNextDisplay == 1) && (nState != _Const.SYNC_STATUS_DEL && mLyricsPlay.m_data.getListSyncTag().get(m_itor).nLineDisplay < mLyricsPlay.m_data.getListLyricsTag().size() - 1)) {
						...
						// if (IKaraokeTV.DEBUG) _LOG.i("BGKIM", "[pos] : " + String.valueOf(pos));
						// if (IKaraokeTV.DEBUG) _LOG.i("BGKIM", "[pos] : " + m_strLine[pos]);
					}
		...
		if (getLinePos() != m_line) {
			...
			if (!mLyricsPlay.m_bSkipChangeColor) {
				...
				if (mLyricsPlay.m_strContinueLyrics != m_strLine[m_line]) {
					...
					// if (IKaraokeTV.DEBUG) _LOG.i("BGKIM", "[m_line] 01 : " + String.valueOf(m_line));
					// if (IKaraokeTV.DEBUG) _LOG.i("BGKIM", "[m_line] 01 : " + m_strLine[m_line]);
				...
				if (mLyricsPlay.m_strContinueLyrics == m_strLine[m_line]) {
					...
					// if (IKaraokeTV.DEBUG) _LOG.i("BGKIM", "[m_line] 02 : " + String.valueOf(m_line));
					// if (IKaraokeTV.DEBUG) _LOG.i("BGKIM", "[m_line] 02 : " + m_strLine[m_line]);
	...
	public void rePaint(int nTime) {
		...
		if (find) {
			if (IKaraokeTV.DEBUG) Log.i("rePaint()", "OK()" + nState + "()" + nCodeFlag + "()" + m_itor + "()" + mLyricsPlay.m_data.getListSyncTag().size() + "()" + nTime + "()" + start + "()" + end + "()" + m_nEndTime + "()");
		} else {
			if (IKaraokeTV.DEBUG) Log.i("rePaint()", "NG()" + nState + "()" + nCodeFlag + "()" + m_itor + "()" + mLyricsPlay.m_data.getListSyncTag().size() + "()" + nTime + "()" + start + "()" + end + "()" + m_nEndTime + "()");
		}
