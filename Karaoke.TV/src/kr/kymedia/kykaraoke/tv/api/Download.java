...
class Download extends Thread implements _Const {
	...
	public void run() {
		...
		if (IKaraokeTV.DEBUG) Log.i(CLASSNAME, "httpget mUrl = " + mUrl);

		try {
			HttpResponse testResponse = downClient.execute(testHttpGet);
			if (IKaraokeTV.DEBUG) Log.i(CLASSNAME, "excute");

			HttpEntity downEntity = testResponse.getEntity();
			if (downEntity != null) {
				if (IKaraokeTV.DEBUG) Log.i(CLASSNAME, "entity not null");
				...
				if (IKaraokeTV.DEBUG) Log.i(CLASSNAME, "getcontent");
				...
				file.createNewFile();
				if (IKaraokeTV.DEBUG) Log.i(CLASSNAME, "createnewfile");
				...
				while ((readSize = testInputBuf.read(buffer)) != -1) {
					if (IKaraokeTV.DEBUG) Log.i(CLASSNAME, "readSize = " + String.valueOf(readSize));
					testOutputBuf.write(buffer, 0, readSize);
				}

				switch (fileType) {
				case REQUEST_FILE_ARTIST_IMAGE:
					if (IKaraokeTV.DEBUG) Log.i(CLASSNAME, "_COMPLETE_ARTIST_IMAGE");
					sendMessage(COMPLETE_DOWN_ARTIST_IMAGE);
					break;
				case REQUEST_FILE_SONG:
					if (IKaraokeTV.DEBUG) Log.i(CLASSNAME, "_COMPLETE_SONG");
					sendMessage(COMPLETE_DOWN_SONG);
					break;
				case REQUEST_FILE_LISTEN:
					if (IKaraokeTV.DEBUG) Log.i(CLASSNAME, "_COMPLETE_LISTEN");
					sendMessage(COMPLETE_DOWN_LISTEN);
					break;
				case REQUEST_FILE_LISTEN_OTHER:
					if (IKaraokeTV.DEBUG) Log.i(CLASSNAME, "_COMPLETE_LISTEN_OTHER_DOWN");
					sendMessage(COMPLETE_DOWN_LISTEN_OTHER);
					break;
				}

				if (IKaraokeTV.DEBUG) Log.i(CLASSNAME, "write end");
				testOutputBuf.flush();
			...
			testHttpGet.abort();
			if (IKaraokeTV.DEBUG) Log.i(CLASSNAME, "execute fail");
		}
	}
