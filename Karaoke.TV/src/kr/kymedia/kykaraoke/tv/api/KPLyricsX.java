...
class KPLyricsX extends KPLyrics {
	...
	KPLyricsX(_LyricsPlay lyricsPlay) {
		...
		if (IKaraokeTV.DEBUG) Log.i(_toString(), getMethodName() + text);
	}
	 ...
	 *
	 * @see KPLyrics#outRect(android.graphics.Canvas, android.graphics.Paint, int, int, int, int, int, int)
	 */
	private void outRect(Canvas canvas, Paint paint, int m, int w, int y, int colorTitle, int colorName) {
		...
		// if (IKaraokeTV.DEBUG) _LOG.i(_toString(), getMethodName() + mLyricsPlay.m_iSongInfoPosition + ":" + r1 + ":" + r2);
	 ...
	 * 아주잘~~~헌다이븅신!!!이러니까값이안넘어오지...이래놓고강제로처박고앉았네...
	 *
	 * @see KPLyrics#setSinger(android.graphics.Canvas, android.graphics.Paint, java.lang.String, java.lang.String, java.lang.String)
	 */
	@Override
	void setSinger(Canvas canvas, Paint paint, String str1, String str2, String str3) {
		...
		if (IKaraokeTV.DEBUG) Log.i(_toString(), getMethodName() + strSinger + ":" + r1);
		if (IKaraokeTV.DEBUG) Log.i(_toString(), getMethodName() + strComposer + ":" + r2);
		if (IKaraokeTV.DEBUG) Log.i(_toString(), getMethodName() + strLyricist + ":" + r3);
		...
		// w = r1.width();
		// rect = r1;
		// if (IKaraokeTV.DEBUG) _LOG.e(_toString(), getMethodName() + ":" + metrics.density + ":" + metrics.densityDpi + ":" + w + ":" + rect);
		...
		// if (IKaraokeTV.DEBUG) _LOG.e(_toString(), getMethodName() + w + ":" + rect + "\n" + strSinger + "\n" + strComposer + "\n" + strLyricist);
	 ...
	 * 아주잘~~~헌다이븅신!!!이러니까값이안넘어오지...이래놓고강제로처박고앉았네...
	 *
	 * @see KPLyrics#setLyrics(android.graphics.Canvas, android.graphics.Paint, java.lang.String, int, int, int)
	 */
	@Override
	void setLyrics(Canvas canvas, Paint paint, String str, int type, int inColor, int outColor) {
		...
		// if (IKaraokeTV.DEBUG) _LOG.w(_toString(), getMethodName() + "아주잘~~~헌다이븅신!!!" + "(" + "h:" + h + ", l:" + l + ")" + "-" + str);
	...
	public void draw() {
		...
		try {
			if (!mLyricsPlay.m_redraw) {
				// if (IKaraokeTV.DEBUG) _LOG.i(_toString(), "run()" + surfaceHolder);
