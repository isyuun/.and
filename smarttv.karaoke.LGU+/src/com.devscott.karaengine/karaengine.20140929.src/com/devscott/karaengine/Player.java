...
public class Player
{
    ...
    void next_lyric_dump()
    {
        ...
        dump.step = (char[])make.step.clone();
        dump.xz = make.xz;
        dump.yz = make.yz;
    ...
    void erase_char_setup(Global gInst)
    {
        ...
        if(mp.tick.tick() > gInst.fp.madi_tick.tick())
            gInst.fp.char_tick = gInst.fp.madi_tick.tick();
        else
    ...
    int lyrictask(Global gInst, int no, long gsaddr)
    {
        ...
        if(gInst.fp.erase_timer > 0L && --gInst.fp.erase_timer == 0L)
            gInst.disp.disp_send(1029, 0L, 0L);
        if(gInst.fp.intv_logo_timer > 0L)
        {
            ...
            {
                if((long)(++gInst.fp.intv_logo_counter) >= gInst.fp.madi_tick.tick() * 2L)
                {
                    gInst.disp.disp_send(1034, 0L, 0L);
                    gInst.fp.intv_logo_counter = 0;
                }
        ...
        if(gInst.fp.note4_timer > 0L)
        {
            ...
            if(gInst.fp.note4_timer % (long)gInst.fp.timebase == 0L && gInst.fp.dlyric[0].x > 0)
            {
                gInst.fp.note4_x = gInst.fp.dlyric[0].x;
                gInst.fp.note4_y = gInst.fp.dlyric[0].y - 38;
                if((gInst.ctrl_status & 0x10) != 16)
                    gInst.disp.disp_send(1037, 0L, (int)(gInst.fp.note4_timer / (long)gInst.fp.timebase));
            }
            if(gInst.fp.note4_timer == 0L)
    ...
    void madi_tick_update()
    {
        ...
        if(gInst.fp.madi_cur_tick.tick() >= gInst.fp.madi_tick.tick())
        {
            ...
            int spand_tick;
            for(spand_tick = (int)((long)gInst.madi_count * gInst.fp.madi_tick.tick()); (long)spand_tick > gInst.fp.chord_cmd[chord_progress].start_tick; chord_progress++);
            int i = 0;
            for(; (long)spand_tick + gInst.fp.madi_tick.tick() > gInst.fp.chord_cmd[chord_progress].start_tick; chord_progress++);
        }
    }
    ...
    void scrolling_lyric(Global gInst)
    {
        gInst.fp.cur_tick++;
        int w;
        if(gInst.fp.char_tick > (long)gInst.fp.char_width)
        {
            int t = (int)(gInst.fp.char_tick / (long)gInst.fp.char_width);
            if(gInst.fp.char_tick % (long)gInst.fp.char_width > 0L)
                t++;
            if(gInst.fp.cur_tick < (long)t)
                return;
            w = 1;
        } else
        {
            w = (int)((long)gInst.fp.char_width / gInst.fp.char_tick);
            if((long)gInst.fp.char_width % gInst.fp.char_tick > 0L)
                w++;
        }
            ...
            LY _ls, LY _rs1, LY _rs2, char step[], boolean rubi_present, boolean korea_rubi, boolean japan_rubi)
    {
        ...
        while(ls.valid()) 
            ...
            if(ls.at() >= 65 && ls.at() <= 90 || ls.at() >= 97 && ls.at() <= 122)
            {
                int rx = x;
    ...
    private static final String TAG = com/devscott/karaengine/Player.getSimpleName();
    private Lock lock;
