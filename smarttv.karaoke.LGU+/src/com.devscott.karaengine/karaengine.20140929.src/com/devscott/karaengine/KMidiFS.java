...
public class KMidiFS
{
    ...
    {
        for(int mh = 0; mh < 2; mh++)
            if(m_song_handle[mh].hsong_no == (long)sno && (m_song_handle[mh].status & 1) == 1)
            {
                if(m_song_handle[mh].file_handle.valid())
                    KFileIO.dclose(m_song_handle[mh].file_handle);
                m_song_handle[mh] = new song_t();
    ...
    public int mfs_next(int song_no)
    {
        ...
        long i;
        for(i = m_start_sno; i < (long)m_end_sno; i++)
        {
            if((long)(++song_no) == 8888L)
                song_no++;
    ...
    public int mfs_open(long song_no, boolean mode)
    {
        ...
        do
        {
            ...
            if(!mode && m_nesting == 0)
            {
                ...
                if(mh < 2)
                {
                    ...
                    table.buf_base = (long)mh * 0x3d800L + 1L;
                    Etc.enable();
                    mh++;
                } else
                {
        ...
        if((table.song.country == 3 || table.song.country == 17) && table.song.lyric_len > 0L)
        {
            ...
            for(int i = j = 0; (long)i < table.song.lyric_len;)
            {
        ...
        table.save_trk = (mtrk_t[])table.imidi.clone();
        for(int i = 0; i < 71; i++)
            try
            {
        ...
        if(table.rhythm_present)
        {
            ...
            for(int i = ch = 0; i < table.mtrks; i++)
                if(table.save_trk[i].type != '\uFFFF')
                {
                    ...
                    if((ch & 2) != 2 && table.save_trk[i].type == '\t')
                    {
                        table.imidi[j].type = ')';
                        j++;
                        ch |= 2;
                    } else
                    if((ch & 4) != 4 && table.save_trk[i].type == '\n')
                    {
                        table.imidi[j].type = '*';
                        j++;
                        ch |= 4;
                    } else
                    if((ch & 8) != 8 && table.save_trk[i].type == '\021')
                    {
                        table.imidi[j].type = '1';
                        j++;
                        ch |= 8;
                    } else
                    if((ch & 0x10) != 16 && table.save_trk[i].type == '\031')
                    {
                        table.imidi[j].type = '9';
                        j++;
                        ch |= 0x10;
                    } else
                    if((ch & 0x20) != 32 && table.save_trk[i].type == '\032')
                    {
                        table.imidi[j].type = ':';
                        j++;
                        ch |= 0x20;
        ...
        for(int i = 0; i < table.mtrks; i++)
        {
            ...
            if(mp.type < ' ')
            {
                ...
                if(table.song.country == 14)
                {
                    if(mp.type == 0 && table.melody_type1 == 65534)
                        table.melody_type1 = mp.type;
                    else
                    if(mp.type == '\020' && table.melody_type2 == 65534)
                        table.melody_type2 = mp.type;
                } else
                if(mp.type == '\003' && table.melody_type1 == 65534)
                    table.melody_type1 = mp.type;
                else
                if(mp.type == '\023' && table.melody_type2 == 65534)
                    table.melody_type2 = mp.type;
            } else
            if((mp.type & 0xfff0) == 64)
            {
                mp.mode |= 8;
                mp.para = '\0';
                mp.patch = '2';
    ...
    void MidiPrepare(song_t table, long song_no)
    {
        ...
        long pos = table.song.midi_pos.pos() + (long)midih.sizeof();
