...
public class KTrack
{
    ...
    void midi_track_parsing(song_t table, long song_no)
    {
        ...
        for(int i = 0; i < table.mtrks; i++)
            Log.i("ke", (new StringBuilder("   ")).append(i).append(" : Addr=").append(table.imidi[i].addr.pos()).append(" Mode=").append(table.imidi[i].mode).append(" Type=").append(table.imidi[i].type).append("(").append(GetTypeName(table.imidi[i].type)).append(")").toString());
        ...
        for(int i = 0; i < table.ctrl_cnt; i++)
            if(table.ctrl[i].tick < (long)def.RHYTHM_START_TICK())
    ...
    public char get_track_type(int tno, long _pos, short country)
    {
        ...
        while(pos.valid()) 
        {
            ...
            default:
                ...
                case 247: 
                    ...
                    if(tno == 0)
                    {
                        ...
                        if(!KString.memcmp(xcmd, Data.channel4_cmd, 8))
                            CHANNEL4 = true;
                    }
                    pos.inc(length);
                    break;
            ...
            if(channel == '\017' && port == '\017' && ctrl < '\b' && ctrl != '\002')
                type = (char)(272 + ctrl);
            else
            if(channel >= '\001' && channel < '\005' && port == '\004')
                type = (char)(64 + channel);
            else
            if(channel < '\020' && port < '\004')
                type = (char)(port * 16 + channel);
            if(type != '\uFFFF')
                break;
        }
            ...
            default:
                ...
                case 255: 
                    ...
                    case 81: // 'Q'
                    {
                        ...
                        pos.inc(length - 3L);
                        tick_us = tempo_us / (long)table.timebase;
