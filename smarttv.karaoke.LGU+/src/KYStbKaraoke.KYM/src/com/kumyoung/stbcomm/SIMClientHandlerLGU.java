...
import ac.rs.lgd.svc.sim.ConnectionThread;
...
public class SIMClientHandlerLGU extends Activity  {
...
//	  	serverTAG		= "nonhing";
	...
	public static String getTodayFull()
	{
		...
		int min = today.get(today.MINUTE);
	 ...
	 * 서비스 사용자 인증 
	 */
	static public boolean sendRequestServiceUserData()
	{
...
///		return "20120314";	
	...
	public static void requestHostID( ContentResolver cr)
	{
		...
		if (nCursor!=null)
		{
			...
			{
				Log.d(TAG, "HOST_ID :" + nCursor.getString(0) ); // hostId
	...
	- ResponseData : 	ITV R/S ----> STB로 전송하는 응답메시지 객체 
	  ...
	  C : filds
		...
		예) 서비스 사용자 인증 응답
    		...
    		Log.d("sim", "폴 보기 번호 = "+ htCode.get("case_no"));
    		Log.d("sim", "해당 보기의 참여자 수 = "+ htCode.get("case_result"));
...
*/    		
	 ...
	 */
	static public boolean user_regist_sendRequestData() {
		RequestData 	requestdata  = new RequestData();
		ResponseData 	responsedata = new ResponseData();
		try {
            //요청데이타 생성
 ...
 */
	static public void subscription_register_sendRequestData() {
		...
		try {
			...
			returnList = (ArrayList)responsedata.fields.get("return_list");http://cafe.naver.com/ArticleRead.nhn?clubid=10050146&page=1&menuid=0&inCafeSearch=true&searchBy=0&query=3gs&includeAll=&exclude=&include=&exact=&searchdate=all&media=0&sortBy=date&articleid=99928772&referrerAllArticles=true
