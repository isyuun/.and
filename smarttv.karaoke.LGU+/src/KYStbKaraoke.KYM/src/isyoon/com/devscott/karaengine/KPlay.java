...
import com.kumyoung.gtvkaraoke.ContentsDownloader;
...
public class KPlay
{
	class PlayThread extends Thread
	{
		...
		public void run()
		{
			...
			do
			{
				...
				else if (!KEnvr.is(256) && !KEnvr.is(260))
					label0:
	...
	private void task_envr_song(int msg)
	{
		...
		case 769:
			...
			int ret = DataHandler.GetSongInfoFromKY(sno);
	...
	private void song_end_proc(int msg)
	{
		...
		if (msg == 770)
		{
			if (Global.Inst().app != null)
				Global.Inst().app.doMenu(352);
			reset_song(257);
		} else if (!KScore.Inst().setScoreMode().booleanValue())
		{
			if (Global.Inst().app != null)
				Global.Inst().app.doMenu(352);
			reset_song(257);
	...
	private static final String TAG = KPlay.class.getSimpleName();
	PlayThread mThread;
