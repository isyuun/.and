...
class Etc
{
	...
	static void set_play_timer()
	{
		...
		{
			long rate = 0x3938700L / (Global.Inst().fp.tempo_us / Global.Inst().fp.timebase);
			Global.Inst().std_tick = (int) (KOS.TIMER_FREQ * 60L) / (int) rate;
			Log.d("ke", (new StringBuilder("TIME TICK : ")).append(0x3938700L / rate).append(" uS (TIMEBASE ").append(Global.Inst().fp.timebase).append(" TEMPO ").append(0x3938700L / Global.Inst().fp.tempo_us).append(" RATE ").append(rate)
					.append(" )").toString());
			return;
		}
	}
