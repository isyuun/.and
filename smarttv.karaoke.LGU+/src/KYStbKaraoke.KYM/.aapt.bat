...
if exist %filename%.cnt.txt del "%filename%.cnt.txt"

@ECHO on
@ECHO [확인][파일생성]
copy /Y "%1" "%filename%.apk"
aapt dump badging "%filename%.apk" >> "%filename%.txt"
java -jar .dex-method-counts.jar "%filename%.apk" >> "%filename%.cnt.txt"
@ECHO [확인][파일날짜]
powershell (ls "%filename%.txt").LastWriteTime = (Get-Item "%filename%.apk").LastWriteTime
powershell (ls "%filename%.cnt.txt").LastWriteTime = (Get-Item "%filename%.apk").LastWriteTime
REM pause
goto :EOF
:EXIT
