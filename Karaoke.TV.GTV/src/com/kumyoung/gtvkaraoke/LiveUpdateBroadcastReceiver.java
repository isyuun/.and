...
public class LiveUpdateBroadcastReceiver extends BroadcastReceiver {
	...
	public void onReceive(final Context context, final Intent intent)
	{
		...
		Log.e(_toString() + "[LiveUpdateBroadcastReceiver]", getMethodName() + "action:" + action + ", extras:" + extras);
		...
		if (action.equals("android.lgt.appstore.LIVE_UPDATE_AUTO")) {
			...
			if (isSuccess) {
				...
				if (isUpdate)
				{
					if (receivedPid.equals("Q13011202819")) {
						...
						// Global.Inst().app.doAlertExit(99998, "최신 버전으로 자동 업데이트 중입니다.", "어플리케이션을 다시 실행해주세요.");
						// }
						// // _LOG.e(_toString() + "[LiveUpdateBroadcastReceiver]", getMethodName() + "최신 버전으로 자동 업데이트 중입니다.");
						//
						...
						// _LOG.e(_toString() + "[LiveUpdateBroadcastReceiver]", getMethodName() + "[startService]" + componentName + ":" + intentUpdate + ":" + intentUpdate.getData());
						//
						// isyoon:액티비티에서실행해야한다.
						Log.e(_toString() + "[LiveUpdateBroadcastReceiver]", getMethodName() + "[startLiveUpdateService]" + (context instanceof __Main) + ":" + context);
						if ((context != null) && (context instanceof __Main)) {
							((__Main) context).startLiveUpdateService();
						}
			...
			Log.e(_toString() + "[LiveUpdateBroadcastReceiver]", getMethodName() + text);
		}
