...
public class Main2X extends Main {
	...
	protected _KPRequest KP(_KPRequest KP) {
		...
		//if (BuildConfig.DEBUG) p_domain = P_DOMAIN_ATV_TEST;
		if (BuildConfig.DEBUG) Log.i(_toString() + TAG_MAIN, "KP()" + "[ST]" + p_domain);
		return super.KP(KP);
	}

	@Override
	public void KP(int request, String OP, String M1, String M2) {
		if (BuildConfig.DEBUG) Log.i(_toString() + TAG_MAIN, "KP()" + "[ST]" + REQUEST_KP.get(request) + ", " + OP + ", " + M1 + ", " + M2);
