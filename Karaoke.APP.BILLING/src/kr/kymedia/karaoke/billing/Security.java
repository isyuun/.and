...
public class Security {
	...
	/**
	 * This keeps track of the nonces that we generated and sent to the server. We need to keep track
	 * of these until we get back the purchase state and send a confirmation message back to Android
	 * Market. If we are killed and lose this list of nonces, it is not fatal. Android Market will
	 * send us a new "notify" message and we will re-generate a new nonce. This has to be "static" so
	...
	public static ArrayList<VerifiedPurchase> verifyPurchase(String signedData, String signature) {
		...
		if (!TextUtil.isEmpty(signature)) {
			 ...
			 * 
			 * Instead of just storing the entire literal string here embedded in the program, construct
			 * the key at runtime from pieces or use bit manipulation (for example, XOR with some other
			 * string) to hide the actual key. The key itself is not secret information, but we don't want
			 * to make it easy for an adversary to replace the public key with one of their own and then
			 * fake messages from the server.
			 * 
			 * Generally, encryption keys / passwords should only be kept in memory long enough to perform
			 * the operation they need to perform.
			 */
			// String base64EncodedPublicKey = "your public key here";
			PublicKey key = Security.generatePublicKey(INAPP_PUBLIC_KEY);
		...
		try {
			...
			// The nonce might be null if the user backed out of the iap_msg_buy page.
