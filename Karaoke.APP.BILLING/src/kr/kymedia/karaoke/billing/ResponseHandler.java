...
public class ResponseHandler {
	 ...
	 * in response to the application calling {@link BillingService#checkBillingSupported()}.
	 * 
	 * @param supported
	...
	/**
	 * Starts a new activity for the user to iap_msg_buy an item for sale. This method forwards the
	 * intent on to the PurchaseObserver (if it exists) because we need to start the activity on the
	 * activity stack of the application.
	 * 
	 * @param pendingIntent
	 *          a PendingIntent that we received from Android Market that will create the new
	 *          iap_msg_buy page activity
	 * @param intent
	 *          an intent containing a request id in an extra field that will be passed to the
	 *          iap_msg_buy page activity when it is created
	 */
	public static void iap_msg_buyPageIntentResponse(PendingIntent pendingIntent, Intent intent) {
		if (sPurchaseObserver == null) {
			if (Consts.DEBUG) {
	 ...
	 * to the user via {@link BillingService#requestPurchase(String)}. The BillingService calls this
	 * method after it gets the response. Another way this method can be called is if the user bought
	 * something on another device running this same app. Then Android Market notifies the other
	 * devices that the user has purchased an item, in which case the BillingService will also call
	 ...
	 */
	public static void purchaseResponse(final Context context, final PurchaseState purchaseState, final String productId, final String orderId, final long purchaseTime,
			...
			public void run() {
				...
				synchronized (ResponseHandler.class) {
					if (sPurchaseObserver != null) {
						...
						// 주문번호전달 : developerPayload -> orderId
						sPurchaseObserver.postPurchaseStateChange(purchaseState, productId, quantity, purchaseTime, orderId);
					}
				}
			}
		}).start();
	 ...
	 * All purchase state changes are received in the {@link BillingReceiver} and are handled in {@link Security#verifyPurchase(String, String)}.
	 * 
	 * @param context
	 *          the context
	 * @param request
